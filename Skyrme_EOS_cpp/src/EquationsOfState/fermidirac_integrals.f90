...
contains
  ...
  ! wrt T  
    ...
    enddo 
    if (ABS(fc)/(ABS(n)+1.d-20)>1.d-10) print *, "bad mu ",ABS(fc)/(ABS(n)+1.d-20)
  ...
  !   quadrature integrations:
        ...
        call el_dqleg(fdfunc2_el, 0.d0,  s12, res1, par, 3) 
        call el_dqleg(fdfunc1_el,   s1,   s2, res2, par, 3) 
        call el_dqleg(fdfunc1_el,   s2,   s3, res3, par, 3) 
        call el_dqlag(fdfunc1_el,   s3,   1.d0, res4, par, 3) 
  ...
  !   avoid overflow in the exponentials at large x
        ...
        ! Combine and rescale derivatives
        fout(1,1) = 1.d0 
        fout(2,1) = fdeta 
        fout(3,1) = fdeta2 
        fout(4,1) = fdeta3 
        ...
        subroutine fdfunc2_el(x,par,n,fout)
        include 'implno_luke.dek'
  ...
  !   avoid an overflow in the denominator at large x:
        ...
        subroutine el_dqleg(f,a,b,result,par,n)
        include 'implno_luke.dek'
...
!  
  ...
  !           result - result of the 10-point gauss formula
        ...
        do j=1,SIZE(xg) 
          ...
          call f(absc1, par, n, fval1)
          call f(absc2, par, n, fval2)
        ...
        return
      end subroutine el_dqleg
  
  
        subroutine el_dqlag(f,a,b,result, par ,n)
        include 'implno_luke.dek'
  !

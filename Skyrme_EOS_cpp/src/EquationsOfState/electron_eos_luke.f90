module electron_eos_mod
  implicit none 
contains
  subroutine electron_eos(M,n,T,muo,po,eo,so,mut,pt,et,st,mun,pn,en,sn) 
    
    real(8), intent(in)  :: M,n,T
    real(8), intent(out) :: muo, po, eo, so 
    real(8), intent(out) :: mut, pt, et, st 
    real(8), intent(out) :: mun, pn, en, sn 
    real(8), dimension(3,3) :: mu,p,e,s
    
    real(8), dimension(4,4) :: dn,dmu,dp,de,ds
    real(8) :: g,theta,fac,denom 
    ...
    !-- Bisect first 
    muu = ((3.d0*PI2*ABS(n))**(1.d0/3.d0) + M)*1.1d0
    mul = 0.d0 
    if (n<0.d0) then
      mul = -muu
      muu = 0.d0 
    ...
    fac     = g*ONEOSQRT2PI2*M**3*SQRT(theta)*theta
    farr(2) = 1.5d0/theta
    farr(3) = 1.5d0*0.5d0/theta**2 
    farr(4) =-1.5d0*0.5d0*0.5d0/theta**3 
    dn(:,:) = (fd(1,:,:) - afd(1,:,:)) + theta*(fd(2,:,:) - afd(2,:,:))
    dn(:,2) = dn(:,2)                  + 1.0d0*(fd(2,:,1) - afd(2,:,1))
    dn(:,3) = dn(:,3)                  + 2.0d0*(fd(2,:,2) - afd(2,:,2))
    dn(:,4) = dn(:,4)                  + 3.0d0*(fd(2,:,3) - afd(2,:,3))
    
    dn = fac*dn
    dn(:,4) = (dn(:,4) + 3.d0*dn(:,3)*farr(2) + 3.d0*dn(:,2)*farr(3) + dn(:,1)*farr(4))/M**3
    dn(:,3) = (dn(:,3) + 2.d0*dn(:,2)*farr(2) +      dn(:,1)*farr(3))/M**2 
    dn(:,2) = (dn(:,2) +      dn(:,1)*farr(2))/M  
    
    fac     = 2.d0/3.d0*g*ONEOSQRT2PI2*M**4*SQRT(theta)*theta**2 
    farr(1) = fac 
    farr(2) = 2.5d0/theta
    farr(3) = 2.5d0*1.5d0/theta**2 
    farr(4) = 2.5d0*1.5d0*0.5d0/theta**3 
    dp(:,:) = (fd(2,:,:) + afd(2,:,:)) + theta*0.5d0*(fd(3,:,:) + afd(3,:,:))
    dp(:,2) = dp(:,2) + 0.5d0*(fd(3,:,1) + afd(3,:,1))
    dp(:,3) = dp(:,3) + 1.0d0*(fd(3,:,2) + afd(3,:,2))
    dp(:,4) = dp(:,4) + 1.5d0*(fd(3,:,3) + afd(3,:,3))
     
    dp = dp*fac
    dp(:,4) = (dp(:,4) + 3.d0*dp(:,3)*farr(2) + 3.d0*dp(:,2)*farr(3) + dp(:,1)*farr(4))/M**3
    dp(:,3) = (dp(:,3) + 2.d0*dp(:,2)*farr(2) + dp(:,1)*farr(3))/M**2  
    dp(:,2) = (dp(:,2) + dp(:,1)*farr(2))/M   
    ...
    farr(3) = 1.5d0*0.5d0/theta**2 
    farr(4) =-1.5d0*0.5d0*0.5d0/theta**3 
    de(:,:) = ((fd(1,:,:) + afd(1,:,:)) + 2.d0*theta*(fd(2,:,:) + afd(2,:,:)) + theta**2*(fd(3,:,:) + afd(3,:,:)))
    de(:,4) = de(:,4) + 6.d0*(fd(2,:,3) + afd(2,:,3)) + 6.d0*theta*(fd(3,:,3) + afd(3,:,3)) + 6.d0*(fd(3,:,2) + afd(3,:,2)) 
    de(:,3) = de(:,3) + 4.d0*(fd(2,:,2) + afd(2,:,2)) + 4.d0*theta*(fd(3,:,2) + afd(3,:,2)) + 2.d0*(fd(3,:,1) + afd(3,:,1)) 
    de(:,2) = de(:,2) + 2.d0*(fd(2,:,1) + afd(2,:,1)) + 2.d0*theta*(fd(3,:,1) + afd(3,:,1))  
    
    de = de*fac 
    de(:,4) = (de(:,4) + 3.d0*de(:,3)*farr(2) + 3.d0*de(:,2)*farr(3) + de(:,1)*farr(4))/M**3 
    de(:,3) = (de(:,3) + 2.d0*de(:,2)*farr(2) + de(:,1)*farr(3))/M**2  
    de(:,2) = (de(:,2) + de(:,1)*farr(2))/M  
    
    !-- Add in the mass contribution to the energy density 
    !de = de + M*dn
     
    ! This is actually eta 
    dmu(:,:) = 0.d0 
    dmu(1,1) = (muc-M)/T 
    dmu(2,1) = 1.d0
    dmu(1,2) = 0.d0 
    ...
    p(3,1) =(   dn(1,2)*dn(3,1)*dp(2,1) + dn(2,1)**2*dp(2,2)            & 
             -  dn(2,1)*(dn(2,2)*dp(2,1) + dn(1,2)*dp(3,1)))/dn(2,1)**3
    p(2,2) =(   dn(1,2)*dn(3,1)*dp(2,1) + dn(2,1)**2*dp(2,2)            & 
             -  dn(2,1)*(dn(2,2)*dp(2,1) + dn(1,2)*dp(3,1)))/dn(2,1)**3
    p(3,2) =(-3*dn(1,2)*dn(3,1)**2*dp(2,1)                              &
             +  dn(2,1)*((3*dn(2,2)*dn(3,1) + dn(1,2)*dn(4,1))*dp(2,1)  &
             +3*dn(1,2)*dn(3,1)*dp(3,1)) + dn(2,1)**3*dp(3,2)           & 
             -  dn(2,1)**2*(dn(3,2)*dp(2,1) + dn(3,1)*dp(2,2)           &
             +2*dn(2,2)*dp(3,1) + dn(1,2)*dp(4,1)))/dn(2,1)**5
    
    e(1,1) = de(1,1) 
    e(2,1) = de(2,1)/dn(2,1) 
    e(1,2) = (de(1,2) - de(2,1)*dn(1,2)/dn(2,1))
    e(3,1) = (   dn(1,2)*dn(3,1)*de(2,1) + dn(2,1)**2*de(2,2)            & 
              -  dn(2,1)*(dn(2,2)*de(2,1) + dn(1,2)*de(3,1)))/dn(2,1)**3
    e(2,2) = (   dn(1,2)*dn(3,1)*de(2,1) + dn(2,1)**2*de(2,2)            & 
              -  dn(2,1)*(dn(2,2)*de(2,1) + dn(1,2)*de(3,1)))/dn(2,1)**3
    e(3,2) = (-3*dn(1,2)*dn(3,1)**2*de(2,1)                              &
              +  dn(2,1)*((3*dn(2,2)*dn(3,1) + dn(1,2)*dn(4,1))*de(2,1)  &
              +3*dn(1,2)*dn(3,1)*de(3,1)) + dn(2,1)**3*de(3,2)           & 
              -  dn(2,1)**2*(dn(3,2)*de(2,1) + dn(3,1)*de(2,2)           &
              +2*dn(2,2)*de(3,1) + dn(1,2)*de(4,1)))/dn(2,1)**5
    
    mu(1,1) = dmu(1,1) 
    mu(2,1) = dmu(2,1)/dn(2,1)
     
    mu(1,2) = (dmu(1,2) - dmu(2,1)*dn(1,2)/dn(2,1))
    
    mu(3,1) = (   dn(1,2)*dn(3,1)*dmu(2,1) + dn(2,1)**2*dmu(2,2)            & 
               -  dn(2,1)*(dn(2,2)*dmu(2,1) + dn(1,2)*dmu(3,1)))/dn(2,1)**3
    mu(2,2) = (   dn(1,2)*dn(3,1)*dmu(2,1) + dn(2,1)**2*dmu(2,2)            & 
               -  dn(2,1)*(dn(2,2)*dmu(2,1) + dn(1,2)*dmu(3,1)))/dn(2,1)**3
    mu(3,2) = (-3*dn(1,2)*dn(3,1)**2*dmu(2,1)                               &
               +  dn(2,1)*((3*dn(2,2)*dn(3,1) + dn(1,2)*dn(4,1))*dmu(2,1)   &
               +3*dn(1,2)*dn(3,1)*dmu(3,1)) + dn(2,1)**3*dmu(3,2)           & 
               -  dn(2,1)**2*(dn(3,2)*dmu(2,1) + dn(3,1)*dmu(2,2)           &
               +2*dn(2,2)*dmu(3,1) + dn(1,2)*dmu(4,1)))/dn(2,1)**5
    
    !-- Switch from eta to chemical potential 
    mu(:,2) = mu(:,2)*T + mu(:,1)
    mu(:,1) = mu(:,1)*T 
    mu(1,1) = mu(1,1)   + M 
  ...
  contains
    ...
    function electron_set_fd(muin,dntdmu) result(nt) 
      ...
      call dfermi(0.5d0,denom,aetat,theta, afd(1,1,1), afd(1,2,1), afd(1,1,2), afd(1,3,1), afd(1,1,3), &
                                           afd(1,4,1), afd(1,1,4), afd(1,2,2), afd(1,3,2), afd(1,2,3))  
      call dfermi(1.5d0,denom,aetat,theta, afd(2,1,1), afd(2,2,1), afd(2,1,2), afd(2,3,1), afd(2,1,3), & 
                                           afd(2,4,1), afd(2,1,4), afd(2,2,2), afd(2,3,2), afd(2,2,3))  
      call dfermi(2.5d0,denom,aetat,theta, afd(3,1,1), afd(3,2,1), afd(3,1,2), afd(3,3,1), afd(3,1,3), & 
                                           afd(3,4,1), afd(3,1,4), afd(3,2,2), afd(3,3,2), afd(3,2,3))  
                                                                                                                              
      !-- Transform derivatives of anti-particles     
      afd(:,1,4) = (12.d0*theta**2*afd(:,2,1) - 24.d0*theta*afd(:,3,1) + 8.d0*afd(:,4,1) - 12.d0*theta**3*afd(:,2,2) &
                  + 12.d0*theta**2*afd(:,3,2) + 6.d0*theta**4*afd(:,2,3) + theta**6*afd(:,1,4))/theta**6  
       
      afd(:,1,3) = afd(:,1,3) + 4.d0*afd(:,2,2)/theta**2 + 4.d0*afd(:,3,1)/theta**4 - 4.d0*afd(:,1,2)/theta**3  
      afd(:,2,3) = afd(:,2,3) + 4.d0*afd(:,3,2)/theta**2 + 4.d0*afd(:,4,1)/theta**4 - 4.d0*afd(:,2,2)/theta**3  

      afd(:,1,2) = afd(:,1,2) + 2.d0/theta**2*afd(:,2,1) 
      afd(:,2,2) = afd(:,2,2) + 2.d0/theta**2*afd(:,3,1) 
      afd(:,3,2) = afd(:,3,2) + 2.d0/theta**2*afd(:,4,1) 
      
      afd(:,2,:) = -afd(:,2,:)
      afd(:,4,:) = -afd(:,4,:)
       
      nt     = ((fd(1,1,1) - afd(1,1,1)) + theta*(fd(2,1,1) - afd(2,1,1))) 
      dntdmu = ((fd(1,2,1) - afd(1,2,1)) + theta*(fd(2,2,1) - afd(2,2,1)))/T 
  ...
  ! routine el_dqleg010 does 10 point gauss-legendre integration  9 fig accuracy
  ! routine el_dqleg020 does 20 point gauss-legendre integration 14 fig accuracy
  ! routine el_dqleg040 does 40 point gauss-legendre integration 18 fig accuracy
  ! routine el_dqleg080 does 80 point gauss-legendre integration 32 fig accuracy
  ! routine el_dqlag010 does 10 point gauss-laguerre integration  9 fig accuracy
  ! routine el_dqlag020 does 20 point gauss-laguerre integration 14 fig accuracy
  ...
  ! routine el_dqlag080 does 80 point gauss-laguerre integration 32 fig accuracy
        ...
        subroutine dfermi(dk,denom,eta,theta,fd,fdeta,fdtheta, &
  ...
  ! declare
        ...
        double precision dk,denom,eta,theta,fd,fdeta,fdtheta, &
  ...
  ! 14 significant figure accuracy
        call el_dqleg020(fdfunc2_el, 0.d0,  s12, res(1), drde(1), drdt(1), & 
                     drde2(1), drdt2(1), drde3(1), drdt3(1), drdet(1), drde2t(1), drdet2(1), par,3)
        call el_dqleg020(fdfunc1_el,   s1,   s2, res(2), drde(2), drdt(2), & 
                     drde2(2), drdt2(2), drde3(2), drdt3(2), drdet(2), drde2t(2), drdet2(2), par,3)
        call el_dqleg020(fdfunc1_el,   s2,   s3, res(3), drde(3), drdt(3), & 
                     drde2(3), drdt2(3), drde3(3), drdt3(3), drdet(3), drde2t(3), drdet2(3), par,3)
        call el_dqlag020(fdfunc1_el,   s3, 1.d0, res(4), drde(4), drdt(4), &
                     drde2(4), drdt2(4), drde3(4), drdt3(4), drdet(4), drde2t(4), drdet2(4), par,3)
  !  9 significant figure accuracy switch to 010
  ! 14 significant figure accuracy switch to 020
  ! 18 significant figure accuracy switch to 040
  ! 32 significant figure accuracy switch to 080
  ...
  ! sum the contributions
        ...
        fdeta3       = drde3(1)  + drde3(2)  + drde3(3)  + drde3(4)
        fdtheta3     = drdt3(1)  + drdt3(2)  + drdt3(3)  + drdt3(4)
        fdetadtheta  = drdet(1)  + drdet(2)  + drdet(3)  + drdet(4)
        fdeta2dtheta = drde2t(1) + drde2t(2) + drde2t(3) + drde2t(4)
        fdetadtheta2 = drdet2(1) + drdet2(2) + drdet2(3) + drdet2(4)
        ...
        subroutine fdfunc1_el(x,par,n,fd,fdeta,fdtheta,    &
                              fdeta2,fdtheta2,fdeta3,fdtheta3,fdetadtheta, &
  ...
  !   avoid overflow in the exponentials at large x
        ...
        subroutine fdfunc2_el(x,par,n,fd,fdeta,fdtheta,   &
                             fdeta2,fdtheta2,fdeta3,fdtheta3,fdetadtheta, & 
                             fdeta2dtheta,fdetadtheta2)
        include 'implno_luke.dek'
  ...
  !   avoid an overflow in the denominator at large x:
        ...
        subroutine el_dqleg010(f,a,b,result,drdeta,drdtheta, &
  ...
  ! declare
        ...
        double precision a,b,result,drdeta,drdtheta, &
                         ...
                         deta2(2),dtheta2(2),deta3(2),dtheta3(2),detadtheta(2),deta2dtheta(2),detadtheta2(2)
  ...
  !           result - result of the 10-point gauss formula
        ...
        do j=1,5
          ...
          call f(absc1, par, n, fval(1), deta(1), dtheta(1), &
                 deta2(1), dtheta2(1), deta3(1), dtheta3(1),detadtheta(1),deta2dtheta(1),detadtheta2(1))
          ...
          drdeta3       = drdeta3   + (deta3(1)   + deta3(2))*wg(j)
          drdtheta3     = drdtheta3 + (dtheta3(1) + dtheta3(2))*wg(j)
          drdeta2dtheta = drdeta2dtheta+(deta2dtheta(1)+deta2dtheta(2))*wg(j)
          drdetadtheta2 = drdetadtheta2+(detadtheta2(1)+detadtheta2(2))*wg(j)
        enddo
        ...
        drdeta3       = drdeta3 * hlfrun
        drdtheta3     = drdtheta3 * hlfrun
        drdeta2dtheta = drdeta2dtheta * hlfrun
        drdetadtheta2 = drdetadtheta2 * hlfrun
        return
      end subroutine el_dqleg010
  
  
  
        subroutine el_dqleg020(f,a,b,result,drdeta,drdtheta, &
                            drdeta2,drdtheta2,drdeta3,drdtheta3,drdetadtheta,drdeta2dtheta,drdetadtheta2,par,n)
        include 'implno_luke.dek'
  !
  ...
  !           result - result of the 20-point gauss formula
        ...
        drdetadtheta2 = drdetadtheta2 * hlfrun
        return
      end subroutine el_dqleg020
  
  
  
  
  
        subroutine el_dqleg040(f,a,b,result,drdeta,drdtheta, &
                            drdeta2,drdtheta2,drdeta3,drdtheta3,drdetadtheta,drdeta2dtheta,drdetadtheta2,par,n)
        include 'implno_luke.dek'
  !
  ...
  !           result - result of the 20-point gauss formula
        ...
        drdetadtheta2 = drdetadtheta2 * hlfrun
        return
      end subroutine el_dqleg040
  
  
  
  
  
        subroutine el_dqleg080(f,a,b,result,drdeta,drdtheta, &
                            drdeta2,drdtheta2,drdeta3,drdtheta3,drdetadtheta,drdeta2dtheta,drdetadtheta2,par,n)
        include 'implno_luke.dek'
  !
  ...
  !           result - result of the 20-point gauss formula
        ...
        drdetadtheta2 = drdetadtheta2 * hlfrun
        return
      end subroutine el_dqleg080
        
  
  
  
  
        subroutine el_dqlag010(f,a,b,result,drdeta,drdtheta, &
                            drdeta2,drdtheta2,drdeta3,drdtheta3,drdetadtheta,drdeta2dtheta,drdetadtheta2,par,n)
        include 'implno_luke.dek'
  !
  ...
  ! declare
        ...
        double precision a,b,result,drdeta,drdtheta, &
                         ...
                         deta2,dtheta2,deta3,dtheta3,detadtheta,deta2dtheta,detadtheta2
  ...
  !           result - result of the 10-point gauss formula
        ...
        do j=1,10
         ...
         drdeta3       = drdeta3   + deta3*wg(j)
         drdtheta3     = drdtheta3 + dtheta3*wg(j)
         drdeta2dtheta = drdeta2dtheta+deta2dtheta*wg(j)
         drdetadtheta2 = drdetadtheta2+detadtheta2*wg(j)
        enddo
        ...
        drdeta3       = drdeta3 * b
        drdtheta3     = drdtheta3 * b
        drdeta2dtheta = drdeta2dtheta * b
        drdetadtheta2 = drdetadtheta2 * b
        return
      end subroutine el_dqlag010
        
  
  
  
  
        subroutine el_dqlag020(f,a,b,result,drdeta,drdtheta, &
                            drdeta2,drdtheta2,drdeta3,drdtheta3,drdetadtheta,drdeta2dtheta,drdetadtheta2,par,n)
        include 'implno_luke.dek'
  !
  ...
  !           result - result of the 20-point gauss formula
        ...
        drdetadtheta2 = drdetadtheta2 * b
        return
      end subroutine el_dqlag020
  
  
  
  
        subroutine el_dqlag040(f,a,b,result,drdeta,drdtheta, &
                            drdeta2,drdtheta2,drdeta3,drdtheta3,drdetadtheta,drdeta2dtheta,drdetadtheta2,par,n)
        include 'implno_luke.dek'
  !
  ! 20 point gauss-laguerre rule for the fermi-dirac function.
  ...
  !           result - result of the 20-point gauss formula
        ...
        drdetadtheta2 = drdetadtheta2 * b
        return
      end subroutine el_dqlag040
  
  
  
  
  
        subroutine el_dqlag080(f,a,b,result,drdeta,drdtheta, &
                            drdeta2,drdtheta2,drdeta3,drdtheta3,drdetadtheta,drdeta2dtheta,drdetadtheta2,par,n)
        include 'implno_luke.dek'
  !
  ! 20 point gauss-laguerre rule for the fermi-dirac function.
  ...
  !           result - result of the 20-point gauss formula
        ...
        drdetadtheta2 = drdetadtheta2 * b
        return
      end subroutine el_dqlag080
end module electron_eos_mod        
